...
import { BarChart3, Search, Sparkles, Wand2 } from "lucide-react";
...
// Return unique recent tickers for fetch_prediction
const getRecentTickers = (
	name: string,
	recent?: Array<{ args: Record<string, any>; at: number }>,
): string[] => {
	if (name !== "fetch_prediction" || !recent) return [];
	return Array.from(
		new Set(
			recent
				.map((r) => (typeof r.args?.ticker === "string" ? r.args.ticker : null))
				.filter((t): t is string => Boolean(t)),
		),
	);
};
...
export default function UsedTools() {
	...
	// Collect unique tickers from most recent tool usages (up to 8 tools x recent args)
	const tickersToFetch = useMemo(() => {
		const set = new Set<string>();
		for (const t of sorted) {
			if (t.name !== "fetch_prediction") continue;
			getRecentTickers(t.name, t.recent).forEach((tk) => {
				set.add(tk);
			});
		}
	...
	// Use per-ticker queries with stable ordering to avoid refetching all when adding new tickers
	const stableTickers = useMemo(() => {
		return [...tickersToFetch].sort();
	}, [tickersToFetch]);

	const tickerQueries = useQueries({
		queries: stableTickers.map((ticker) => ({
			...
			queryFn: async () => {
				...
				if (!res.ok || !json?.data) throw new Error("prediction failed");
				const d = json.data;
				return {
					ticker: d.ticker,
	...
	const predictions = useMemo(() => {
		const map: Record<string, TickerPrediction> = {};
		tickerQueries.forEach((q, idx) => {
			const tk = stableTickers[idx];
			if (q.data) map[tk] = q.data as TickerPrediction;
		});
		return map;
	}, [tickerQueries, stableTickers]);

	if (!sorted.length) return null;

	return (
		<TooltipProvider>
			<div className="flex items-center w-full mb-8 justify-center gap-2">
				<ul
					className={cn(
				...
				>
					<AnimatePresence initial={false}>
						{sorted.map((t) => {
							...
							const tickers = getRecentTickers(t.name, t.recent);
							const queries = getRecentQueries(t.name, t.recent);
							const extraCount = Math.max(0, tickers.length - 6);
									...
									<TooltipTrigger asChild>
										...
										>
											<Icon
												...
												aria-hidden
											/>
											<div className="max-w-[16rem] grid gap-1 overflow-hidden">
												<span className="font-medium text-foreground/90">
													{label}
												</span>
												{t.name === "fetch_prediction" && tickers.length > 0 ? (
													<span className="inline-flex flex-wrap items-center gap-3 align-middle">
														{tickers.map((tk) => {
															const pred = predictions[tk];
															...
															return (
																...
																>
																	<span className="rounded bg-background/60 px-1.5 py-0.5 text-[10px] font-mono text-muted-foreground">
																		{tk}
																	</span>
																	{pred ? (
																		<>
